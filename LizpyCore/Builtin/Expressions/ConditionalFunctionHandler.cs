...
namespace Lizpy.Builtin.Expressions {
    public class ConditionalFunctionHandler : IExpressionFunctionHandler {
        ...
        public ProgramExpression CompileFunction(CompilerState state, ListExpression expression) {
            ...
            switch (function) {
                case "if" when expression.Items.Count != 4:
                ...
                // disabling case statements for now until we have type safety to determine what kind of comparison to perform.
                // // (case value
                // //     match1 result1
                // //     match2 result2
                // //     defaultResult)
                // case "case" when expression.Items.Count < 5:
                //     throw new LizpyInternalCompilationException(
                //         $"Invalid case expression, argument count mismatch. Expected: >= 4, Actual: {expression.Items.Count - 1}",
                //         expression
                //     );
                // case "case": {
                ...
                // //     match2 result2
                // //     defaultResult)
                // case "scase" when expression.Items.Count < 5:
                //     throw new LizpyInternalCompilationException(
                //         $"Invalid case expression, argument count mismatch. Expected: >= 4, Actual: {expression.Items.Count - 1}",
                //         expression
                //     );
                 ...
                 *     defaultResult)
                 */
                case "cond" when expression.Items.Count < 4:
                    throw new LizpyInternalCompilationException(
                        $"Invalid case expression, argument count mismatch. Expected: >= 3, Actual: {expression.Items.Count - 1}",
                        expression
                    );
                case "cond": {
                    var compiledExpressions =
